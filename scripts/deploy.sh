...
exitaz account set --subscription $AZURE_SUBSCRIPTION_ID

cd ./app/frontend
SWA_DEPLOYMENT_TOKEN=$(az staticwebapp secrets list -n $AZURE_STATICWEBSITE_NAME -g $AZURE_RESOURCE_GROUP --query "properties.apiKey" --output tsv)
